'''
This code implements a convolutional autoencoder following VGG16 architecture
Autoencoder consists of classes VGG16_Encoder and VGG16_Decoder
Each of the classes is called with the number of features, the autoencoder shall have in the bottleneck
IMPORTANT: Only works with input where size of each dimension is 2^n where n is at least 64
'''
import numpy as np
import torch
...
Call encoder with number of features that shall be predicted
...
class VGG16_Encoder(nn.Module):
    def __init__(self,num_features: int = 5):
        ...
        self.fc1= nn.Sequential(nn.Linear(16384, 4096), nn.ReLU())#Adapt number of input features here
        self.fc2 = nn.Sequential(nn.Linear(4096, 4096), nn.ReLU())
        self.fc3 = nn.Sequential(nn.Linear(4096, self.num_features))
    ...
    def forward(self, x):
        ...
        return bn, pooling_indices
    
'''
Decoder reconstructs original data from reduced representation
Call decoder with number of features predicted by encoder
...
class VGG16_Decoder(nn.Module):
    def __init__(self, 
        ...
        self.num_features = num_features
        self.fc3= nn.Sequential(nn.Linear(self.num_features, 4096))
        self.fc2 = nn.Sequential(nn.Linear(4096, 4096), nn.ReLU())
        self.fc1 = nn.Sequential(nn.Linear(4096, 16384), nn.ReLU())#Adapt number of output features here
    ...
    def forward(self, bn, pooling_indices):
        reconstructed = self.fc3(bn)
        reconstructed = self.fc2(reconstructed)
        reconstructed = self.fc1(reconstructed)
        reconstructed = reconstructed.reshape(8, 512, 4, 8)#Adapat to expected size [batch_size x channels x width x height] // width and height = width and height of original image *1/(2^5)
        reconstructed = self.unpooling5(reconstructed, indices = pooling_indices[4])
...
Call with number of features in botleneck of encoder
...
class Autoencoder(nn.Module):
    def __init__(self,
        ...
        self.num_features = num_features
        self.Encoder = VGG16_Encoder(num_features)
        self.Decoder = VGG16_Decoder(num_features)
